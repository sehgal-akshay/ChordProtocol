	defmodule ChordStabilizer do
		...
		def handle_cast({:stabilize, current_node, m}, state) do
			...
			if successor_predecessor == current_node do
				if successor != nil do
					ChordStabilizerCoordinator.stabilize successor, m
				end
			else
				...
				fix_finger current_node, m
				
				#Continue stabilization after the fix
				ChordStabilizerCoordinator.stabilize successor_predecessor, m
		...
		defp fix_finger(current_node, m) do 
		...
		def start(start_node, m) do
			...
			ChordStabilizerCoordinator.stabilize start_node, m
	...
	defmodule ChordStabilizerCoordinator do
		...
		def stabilize(start_node, m) do
			GenServer.cast(:chord_stabilizer, {:stabilize, start_node, m})
		end
